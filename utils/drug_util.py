...
def calculate_graph_feat(feat_mat, adj_list):
    assert feat_mat.shape[0] == len(adj_list)#声明该条件为真，检查
    adj_mat = np.zeros((len(adj_list), len(adj_list)), dtype='float32')#维度，维度；返回给定维度的全0数组类型
    for i in range(len(adj_list)):
        nodes = adj_list[i]
        for each in nodes:
            adj_mat[i, int(each)] = 1
    assert np.allclose(adj_mat, adj_mat.T)#比较两个元素是否相近，返回True or False
    x, y = np.where(adj_mat == 1)#符合条件，返回坐标
    adj_index = np.array(np.vstack((x, y)))#vstack按垂直方向堆叠数组成新数组
...
def drug_feature_extract(drug_data):
    ...
    for i in range(len(drug_feat)):
        feat_mat, adj_list = drug_data.iloc[i]#iloc位置索引，取行索引为i，所有列索引的数据。对DataFrame进行操作
